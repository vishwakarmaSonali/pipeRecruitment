...
const FilterModal = ({ isOpen, onClose, onApply, onReset, filters }) => {
  ...
  const handleKeyDown = (e, field) => {
    if (e.key === "Enter" && localFilters[field]?.trim()) {
      setLocalFilters((prevFilters) => ({
        ...prevFilters,
        [field]: "",
        [`${field}List`]: [...(prevFilters[`${field}List`] || []), prevFilters[field].trim()],
      }));
    }
  };
  ...
  const removeItem = (field, index) => {
    setLocalFilters((prevFilters) => ({
      ...prevFilters,
      [`${field}List`]: prevFilters[`${field}List`].filter((_, i) => i !== index),
    }));
  };
      ...
      <div className="p-[20px] flex flex-col h-full">
        ...
        <div className="flex-1 overflow-auto pb-[6px] scroll-width-none mt-[32px] space-y-[6px]">
          <div>
            ...
            />
         {localFilters.jobTitleList?.length>0 && <div className="inputItemsDiv">
              {localFilters.jobTitleList.map((title, index) => (
                <div key={index} className="inputed-item">
                  {title}
                  <button className="ml-2 text-customBlue" onClick={() => removeItem("jobTitle", index)}>✕</button>
                </div>
              ))}
            </div>}
          </div>
          ...
          <div>
            ...
            />
            {localFilters.locationList?.length>0 &&   <div className="inputItemsDiv">
              {localFilters.locationList?.map((loc, index) => (
                <div key={index} className="inputed-item">
                  {loc}
                  <button className="ml-2 text-customBlue" onClick={() => removeItem("location", index)}>✕</button>
                </div>
              ))}
            </div>}
          </div>
        ...
        <CustomDropdown
          ...
          <div>
            ...
            />
             {localFilters.companyList?.length>0 && <div className="inputItemsDiv">
              {localFilters.companyList?.map((companyItem, index) => (
                <div key={index} className="inputed-item">
                  {companyItem}
                  <button className="ml-2 text-customBlue" onClick={() => removeItem("company", index)}>✕</button>
                </div>
              ))}
            </div>}
          </div>
          ...
          <div>
            ...
            />
             {localFilters.majorList?.length>0 && <div className="inputItemsDiv">
              {localFilters.majorList?.map((majorItem, index) => (
                <div key={index} className="inputed-item">
                  {majorItem}
                  <button className="ml-2 text-customBlue" onClick={() => removeItem("major", index)}>✕</button>
                </div>
              ))}
            </div>}
            <input
            ...
            />
            {localFilters.schoolList?.length>0 &&  <div className="inputItemsDiv">
              {localFilters.schoolList?.map((schoolsDataItem, index) => (
                <div key={index} className="inputed-item">
                  {schoolsDataItem}
                  <button className="ml-2 text-customBlue" onClick={() => removeItem("school", index)}>✕</button>
                </div>
              ))}
            </div>}
            <input
            ...
            />
             {localFilters.degreeList?.length>0 && <div className="inputItemsDiv">
              {localFilters.degreeList?.map((degreesItem, index) => (
                <div key={index} className="inputed-item">
                  {degreesItem}
                  <button className="ml-2 text-customBlue" onClick={() => removeItem("degree", index)}>✕</button>
                </div>
              ))}
            </div>}
          </div>
        ...
        <div className="mt-6 flex justify-between space-x-4">
          ...
          <button className="w-1/2 bg-buttonBLue text-white flex justify-center items-center py-[12px] max-h-[40px] rounded-[8px] hover:bg-blue-700 btn-text"  onClick={() => handleApplyFilters()}>
            Filter Search
          </button>
        </div>
      </div>
