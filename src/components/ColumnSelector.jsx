...
const DraggableItem = ({ column, removeColumn }) => {
  ...
  return (
    <>
      ...
      <div
        className={`fixed top-0 right-0 w-[460px] h-screen bg-white shadow-lg p-[20px]  rounded-l-[10px] transition-transform z-50 flex flex-col ${
          isOpen ? "translate-x-0" : "translate-x-full"
        }`}
      >
        {/* Header Section */}
        <div className="flex justify-between items-center ">
          <h2 className="text-xxl font-medium font-ubuntu text-customBlue">Columns</h2>
          <button onClick={onClose} className="text-xxl font-normal font-ubuntu text-customBlue">
        ...
        <div className="flex-1 overflow-y-auto my-3 gap-[12px] space-y-[12px]">
          ...
          <div className="">
            <button
              ...
              onClick={() => setIsExpanded({ ...isExpanded, select: !isExpanded.select })}
            >
              ...
              <DropArrow
                ...
                height={14}
                fill="customBlue"
                className={`w-5 h-5 transition-transform ${isExpanded.select ? "rotate-0" : "rotate-180"}`}
              />
            </button>
            {isExpanded.select && (
             ...
             <button
                ...
                >
                  <span className="text-buttonBLue text-m font-ubuntu">Unselect all</span>

                </button>
                <button>

                  <span className="text-m font-ubuntu text-customBlue">10 of 20</span>
                </button>
                </div>
               {/* ✅ Show all columns (Selected = Ticked, Unselected = Empty) */}
               <div className={`checkbox-list overflow-auto ${isExpanded.select ? "max-h-[540px]":"max-h-[250px] "}`}>
                {filteredColumns.map((column) => (
                  <div key={column} className="column-option flex items-center  cursor-pointer text-m font-ubuntu" onClick={() => handleCheckboxChange(column)}>
          ...
          <div className="">
            <button
              className="w-full text-left p-2 rounded-[8px] border-1 bg-white font-medium text-m font-ubuntu flex items-center justify-between"
              onClick={() => setIsExpanded({ ...isExpanded, arrange: !isExpanded.arrange })}
            >
              Arrange Columns
              <DropArrow
                width={14}
                height={14}
                fill="customBlue"
                className={`w-5 h-5 transition-transform ${isExpanded.arrange ? "rotate-0" : "rotate-180"}`}
              />
            </button>
         
          <div className={`mx-[10px] my-[12px]  overflow-auto${isExpanded.select ? "max-h-[540px]":"max-h-[250px] "}`}>
          {isExpanded.arrange && (
                           ...
                           <DndContext collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
                             <SortableContext items={reorderedColumns} strategy={verticalListSortingStrategy}>
                               {reorderedColumns.map((column) => (
                                 <DraggableItem key={column} column={column} removeColumn={handleRemoveColumn} style={{background:"red"}}/>
                               ))}
                             </SortableContext>
                           </DndContext>
                         </div>
        ...
        <div className="  pt-3 pb-2 flex justify-end space-x-2 bg-white">
          ...
          <button className="px-[14px] py-[10px] max-h-[36px] text-m font-ubuntu rounded-[8px] flex items-center gap-1 w-full justify-center border-1 border-buttonBLue  cursor-pointer" onClick={handleSave}>
              <span className="font-ubuntu text-buttonBLue text-m">Cancel</span>
            </button>
          <button className="px-[14px] py-[10px] max-h-[36px] text-m font-ubuntu rounded-[8px] flex items-center gap-1 w-full justify-center bg-buttonBLue text-buttonBLue cursor-pointer" onClick={handleSave}>
