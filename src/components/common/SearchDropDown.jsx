...
const SearchDropdown = ({
  ...
  const handleSelect = (option) => {
    if (multiSelect) {
      ...
      const updatedSelections = alreadySelected
        ? selectedOptions.filter((item) => item[optionKey] !== option[optionKey]) // Remove if already selected
  ...
  return (
    <div className="relative w-full" ref={dropdownRef}>
      ...
      <CommonTextInput
        type="text"
        value={multiSelect ? searchTerm : selectedOptions?.[optionKey] || searchTerm}
      ...
      {isDropdownOpen && filteredOptions.length > 0 && (
        <div className="absolute w-full bg-white border border-gray-300 shadow-lg rounded-md mt-1 max-h-60 overflow-auto z-50">
           ...
           console.log("{option[iconKey]{option[iconKey]",option[iconKey]);
            ...
            return(
              <div
                ...
                className={`cursor-pointer hover:bg-gray-100 text-sm font-ubuntu text-customBlue p-[12px] rounded-[8px]  flex  items-center gap-[8px]
                  `}
                //   ${
                //   multiSelect && selectedOptions.some(
                //     (item) => item[optionKey] === option[optionKey]
                //   )
              ...
              >
                    {option[iconKey] && (
                <img src={option[iconKey]} alt={option[optionKey]} className="w-5 h-5" />
              )}
                {option[optionKey]}
              </div>
            )
      ...
      {multiSelect && selectedOptions.length > 0 && (
        <div className="flex flex-wrap gap-2 mt-2">
          {selectedOptions.map((option, index) => (
            <div
              key={index}
              className="flex items-center border p-[8px] rounded-[8px] bg-gray-100"
            >
              <span className="text-sm font-ubuntu text-customBlue">{option[optionKey]}</span>
              <button onClick={() => removeSelectedOption(option)} className="ml-2">
